...
export interface CheckboxRootProps extends UseCheckboxProps, React.LabelHTMLAttributes<HTMLLabelElement> {
...
export const CheckboxRoot = forwardRef<HTMLLabelElement, CheckboxRootProps>((props, ref) => {
  ...
  return (
    <CheckboxProvider value={api}>
      <label ref={ref} {...api.rootProps} {...otherProps} htmlFor={api.hasLabel ? api.checkboxId : undefined}>
        <input ref={composeRefs(api.refs.input)} id={api.checkboxId} {...api.hiddenInputProps} />
        {children}
      </label>
    </CheckboxProvider>
  );
...
export interface CheckboxIndicatorProps extends React.HTMLAttributes<HTMLDivElement> {}
