...
function createBridgeForWebView(webRef: RefObject<RNWebView>) {
  ...
  // pnpm peer dependency 중복 설치로 인한 타입 불일치 (런타임 정상 동작)
  const wire = wireWebView(webRef, router, {
    onWebEvent: (event, payload) => {
      ...
      // 이미지 피커 이벤트 처리
      if (event === 'media.pickImage' || event === 'media.pickImages') {
        const imageHandlers = router.getImagePickerHandlers?.();
        if (imageHandlers && imageHandlers[event]) {
  ...
  router.setImagePickerHandlers?.(webRef as any, wire.sendEvent);
