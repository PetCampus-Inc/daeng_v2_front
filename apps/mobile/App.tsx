...
export default function App() {
  ...
  const handleNavigationStateChange = useCallback((state: NavigationState | PartialState<NavigationState> | undefined) => {
    const currentRootRouteName = state?.routes?.[state.index ?? 0]?.name;

    // 외부/상세 Stack에서 뒤로 돌아오면 이전 화면이 남긴 숨김 상태를 복구한다.
    // iOS 스와이프 뒤로가기와 Android 하드웨어 뒤로가기도 onStateChange로 감지된다.
