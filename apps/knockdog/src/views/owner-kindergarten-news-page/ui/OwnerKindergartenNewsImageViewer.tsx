...
interface OwnerKindergartenNewsImageViewerProps {
  isOpen: boolean;
  close: () => void;
  imageUrls: string[];
  initialIndex?: number;
...
function showSaveSuccessToast() {
  const { imageViewer } = ownerKindergartenNewsContent;
 ...
 * - 저장 → OS 저장 + 토스트
 */
function OwnerKindergartenNewsImageViewer({
  isOpen,
  close,
  imageUrls,
  initialIndex = 0,
}: OwnerKindergartenNewsImageViewerProps) {
  const { detail, imageViewer } = ownerKindergartenNewsContent;
...
export { OwnerKindergartenNewsImageViewer };
