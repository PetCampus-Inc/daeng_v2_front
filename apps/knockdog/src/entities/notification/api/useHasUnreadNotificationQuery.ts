...
const notificationsUnreadQueryKey = (userId?: string, audience?: NotificationAudience) =>
...
async function getHasUnreadNotification(audience: NotificationAudience) {
  const response = await getNotifications({ audience, size: 1 });
  return toNotificationListPage(response.data).hasUnread;
}

/** 헤더 알림 아이콘 배지용 — 목록 상세 없이 안읽음 여부만 필요할 때 size 최소값(1)으로 조회 */
function useHasUnreadNotificationQuery({ audience, userId, enabled = true }: UseHasUnreadNotificationQueryOptions) {
  return useQuery({
    queryKey: notificationsUnreadQueryKey(userId, audience),
