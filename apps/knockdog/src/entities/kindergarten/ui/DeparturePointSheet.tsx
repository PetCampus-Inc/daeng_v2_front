...
export function DeparturePointSheet({ isOpen, close, to }: DeparturePointSheetProps) {
  ...
  const label = locationLoading
    ...
    : isLoading
      ...
      : error
        ? '주소를 조회할 수 없습니다'
        : hasValidAddress
          ? (primaryText || primaryRoad || primaryParcel)!
          : locationError
            ? '위치 정보를 가져올 수 없습니다'
            : '위치 정보 없음';

  // 임시 더미 데이터 (나중에 실제 저장된 주소 데이터로 대체)
  ...
  function handleRadioChange(value: string) {
    ...
    console.log('선택된 출발지:', locationName);
    console.log('위도:', selectedCoords?.lat);
    console.log('경도:', selectedCoords?.lng);
    console.log('좌표:', selectedCoords);
