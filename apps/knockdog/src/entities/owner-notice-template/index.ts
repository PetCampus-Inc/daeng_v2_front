export {
  consumeLoadedNoticeTemplateContent,
  peekLoadedNoticeTemplateContent,
  saveLoadedNoticeTemplateContent,
  type CreateOwnerNoticeTemplateInput,
  type OwnerNoticeTemplate,
} from './model/ownerNoticeTemplate';
export { useOwnerNoticeTemplates } from './model/useOwnerNoticeTemplates';
export {
  useOwnerNoticeTemplateDetailQuery,
  useOwnerNoticeTemplatesQuery,
} from './api/useOwnerNoticeTemplateQuery';
export { useOwnerNoticeTemplateMutation } from './api/useOwnerNoticeTemplateMutation';
