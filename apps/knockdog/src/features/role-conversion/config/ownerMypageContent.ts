export const ownerMypageContent = Object.freeze({
  ownerBadgeLabel: '원장',
  accountSectionTitle: '계정 정보',
  ttokIdLabel: '똑똑 ID',
  ttokIdDescription: '문의 시 사용되는 ID예요',
  releasePermissionLabel: '운영 권한 해제하기',
  withdrawBlockedModalTitleLine1: '원장 권한이 있어',
  withdrawBlockedModalTitleLine2: '바로 탈퇴할 수 없어요',
  withdrawBlockedModalDescriptionLine1: '유치원 운영 권한을 먼저 종료한 뒤',
  withdrawBlockedModalDescriptionLine2: '회원 탈퇴를 진행할 수 있어요.',
  withdrawBlockedModalConfirmLabel: '권한 해제하러 가기',
  otherInfoTitle: '기타 정보',
  logoutLabel: '로그아웃',
  withdrawLabel: '탈퇴하기',
  profilePageTitle: '프로필',
  kindergartenPageTitle: '유치원 정보',
  kindergartenOperationTabLabel: '운영',
  kindergartenPricingTabLabel: '요금',
  kindergartenBasicInfoTitle: '기본 정보',
  kindergartenNameLabel: '유치원 명',
  kindergartenAddressLabel: '주소',
  kindergartenPhoneLabel: '유치원 전화번호',
  noConfirmedInfoText: '확인된 정보가 없어요.',
  kindergartenPricingProductTypeTitle: '상품 유형',
  kindergartenPricingPriceListTitle: '가격표',
  kindergartenPricingPriceListEmptyText: '등록된 가격표 사진이 없어요',
  kindergartenPricingServiceTitle: '서비스 및 이용요금',
  kindergartenPricingServiceEmptyText: '등록된 요금 정보가 없어요',
  kindergartenPricingEditPageTitle: '요금 정보 수정',
  kindergartenPricingEditProductTypeLabel: '상품 유형',
  kindergartenPricingEditPriceListLabel: '가격표',
  kindergartenPricingEditServiceNoticeTitle: '요금 변경은 고객센터에 문의해 주세요',
  kindergartenPricingEditServiceNoticeDescription: '확인 후 빠르게 수정해 드릴게요.',
  kindergartenPricingEditCustomerCenterLabel: '고객센터',
  kindergartenEditPageTitle: '운영 정보 수정',
  kindergartenEditBasicSectionTitle: '기본 정보',
  kindergartenEditHoursSectionTitle: '운영시간',
  kindergartenEditSnsSectionTitle: '웹사이트·SNS',
  kindergartenEditDetailsSectionTitle: '세부 정보',
  kindergartenEditImageLabel: '대표 이미지',
  kindergartenEditImagesError: '유치원 사진을 등록해주세요.',
  kindergartenEditRepresentativeBadge: '대표 사진',
  kindergartenEditNameLabel: '유치원 명',
  kindergartenEditAddressLabel: '주소',
  kindergartenEditAddressSearchTitle: '주소 수정하기',
  kindergartenEditAddressSearchPlaceholder: '주소를 검색해주세요',
  kindergartenEditAddressDetailPlaceholder: '상세 주소를 입력해주세요',
  kindergartenEditPhoneLabel: '유치원 전화번호',
  kindergartenEditPhoneFormatError: '전화번호 형식을 확인해 주세요.',
  kindergartenEditWeekdayOperatingHoursError: '운영시간을 확인해 주세요.',
  kindergartenEditWeekendOperatingHoursError: '운영시간을 확인해 주세요.',
  kindergartenEditWeekdayLabel: '평일',
  kindergartenEditWeekendLabel: '주말',
  kindergartenEditClosedDaysLabel: '정기 휴무일',
  kindergartenEditHomepageLabel: '홈페이지',
  kindergartenEditInstagramLabel: '인스타그램',
  kindergartenEditYoutubeLabel: '유튜브',
  kindergartenEditWebAddressFormatError: '주소 형식을 확인해 주세요. ex) www.knockdog.com',
  kindergartenEditBreedTitle: '견종',
  kindergartenEditDogServiceTitle: '강아지 서비스',
  kindergartenEditSafetyTitle: '강아지 안전 시설',
  kindergartenEditAmenityTitle: '방문객 편의·시설',
  kindergartenEditMultiSelectHint: '복수 선택 가능',
  kindergartenEditLastUpdatedTitle: '최종 정보 업데이트',
  kindergartenEditSaveSuccessToastPrefix: '변경사항',
  kindergartenEditSaveSuccessToastSuffix: '이 저장됐어요',
  kindergartenEditSaveSuccessToastFallback: '변경사항이 저장됐어요',
  kindergartenEditTimePlaceholder: '시간 선택',
  kindergartenEditTimeSheetTitle: '시간 선택',
  kindergartenEditClosedDaysPlaceholder: '휴무일 선택',
  kindergartenEditClosedDaysSheetTitle: '휴무일 선택',
  profileEditPageTitle: '프로필 수정하기',
  editInfoButtonLabel: '정보 수정하기',
  ownerNameLabel: '이름',
  ownerPhoneLabel: '연락처',
  ownerEmailLabel: '이메일',
  ownerNamePlaceholder: '이름을 입력해주세요',
  ownerPhonePlaceholder: '연락처를 입력해주세요',
  ownerEmailPlaceholder: '이메일을 입력해주세요',
  ownerPhoneFormatError: '전화번호 형식을 확인해 주세요. ex) 010-1234-5678',
  ownerEmailFormatError: '이메일 형식을 확인해 주세요. ex) knock@dog.com',
  profileSaveButtonLabel: '저장하기',
  unsavedExitModalTitle: '저장하지 않고 나갈까요?',
  unsavedExitModalDescription: '변경한 내용이 저장되지 않아요.',
  unsavedExitModalCancelLabel: '닫기',
  unsavedExitModalConfirmLabel: '나가기',
});

