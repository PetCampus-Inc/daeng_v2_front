...
export function DogSchoolCardSheet({
  isOpen,
  onChangeOpen,
  ...props
}: DogSchoolCardSheetProps) {
  const openPhoneCallActionSheet = () =>
    overlay.open(({ isOpen, close }) => (
      <PhoneCallSheet isOpen={isOpen} close={close} />
    ));

  const openDeparturePointSheet = () =>
    overlay.open(({ isOpen, close }) => (
      <DeparturePointSheet isOpen={isOpen} close={close} />
    ));
  return (
            ...
            <div className='gap-x2 flex flex-1 flex-col'>
              <div className='flex items-start justify-between'>
                ...
                <button onClick={openDeparturePointSheet}>
                  <Icon
                    icon='Navigation'
                    className='text-fill-secondary-500 size-x8'
                  />
                </button>
              </div>
              ...
              <div className='gap-x1 flex'>
                ...
                <div className='px-x2 py-x1 radius-r2 bg-fill-secondary-50 flex shrink-0 items-center gap-[2px]'>
                  ...
                  <span className='caption1-semibold text-text-primary'>
                    2025.04.16
                  </span>
                  <span className='caption1-semibold text-text-primary'>
                    메모
        ...
        <BottomSheet.Footer className='pt-x6 pb-x4'>
          <div className='gap-x2 flex'>
            <ActionButton
              variant='primaryLine'
              size='medium'
              onClick={openPhoneCallActionSheet}
            >
              전화하기
            </ActionButton>
