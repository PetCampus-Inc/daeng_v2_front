...
interface KindergartenCardSheetProps extends DogSchoolWithMeta {
  ...
  onCardExpansion?: () => void;
  onExit?: () => void;
}

const snapPoints = ['328px', 1];

export function KindergartenCardSheet({
  isOpen,
  shouldAnimate = true,
  onCardExpansion,
  onExit,
  ...props
}: KindergartenCardSheetProps) {
  const [snap, setSnap] = useState<number | string | null>(snapPoints[0] ?? null);

  useEffect(() => {
    if (snap === 1 && onCardExpansion) {
      onCardExpansion();
    }
  }, [snap, onCardExpansion]);
  ...
  return (
    ...
    >
      <BottomSheet.Portal>
        <BottomSheet.Body
          className='bottom-[68px] z-50 h-full'
