...
import { PickupType, ServiceTag } from '@entities/kindergarten';
...
function BadgeGroup({ children }: BadgeGroupProps) {
  ...
  return (
    <ul ref={containerRef} className='gap-x1 flex flex-nowrap items-center overflow-hidden'>
      {children.map((child, index) => (
        <li
          ...
          className={cn(
            ...
            hiddenItemIds.includes(index.toString()) && 'pointer-events-none invisible order-last'
          )}
        >
          {child}
        </li>
      ...
      {hiddenItemIds.length > 0 && (
        <span ref={moreButtonRef} className='caption1-semibold text-text-secondary shrink-0'>
