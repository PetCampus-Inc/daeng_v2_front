...
import { BridgeException, type ShareParams } from '@knockdog/bridge-core';
...
function useShare() {
  ...
  return useCallback(
    async function share(params: ShareParams): Promise<boolean> {
      if (isNativeWebView()) {
        try {
          bridge.emit('system.share', params);
