...
function ReportingKindergartenUpdate() {
  ...
  const [newAddress, setNewAddress] = useState<string | null>(roadAddress);
  ...
  const reportingParams = useMemo(() => {
    return {
      address: checkedSet.has('address') ? (newAddress ?? '') : '',
                ...
                <div className='my-5 flex flex-col gap-3 px-4'>
                  ...
                  <div className='mt-5 flex gap-2'>
                    ...
                    <ActionButton asChild variant='secondaryLine' className='flex-1'>
                      <Link href={`/company/${id}/report-info-update/manual-address`}>직접입력</Link>
                    </ActionButton>
                  </div>
                </div>
              )}
